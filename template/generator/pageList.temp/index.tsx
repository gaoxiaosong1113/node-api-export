...
class Template extends React.Component<IDComponentProps, IDComponentState> {
    ...
    public columns = [
        ...
        },
        {
            title: '订单状态',
            dataIndex: 'statusCd',
            render: (t, r) => CodesUtils.getLable('DEALER_ORDER_STATUS', t)
        },
        {
            title: '预警',
            dataIndex: 'earlyWarningStatusCd',
            render: (text, record) => <EarlyWarning model="templateModel/saleOrderGetReason" status={record.earlyWarningStatusCd} orderCode={record.orderCode} code="CG_ORDER_YJ_STATUS" />
        },
        {
            title: '金额',
            dataIndex: 'amount',
    ...
    public render() {
        ...
        return (
            <FormTable
                title="经销采购单"
                table={{
                    models: 'templateModel/saleOrderPage',
                    loading: loading,
                ...
                searchElement={
                    <Form onSubmit={this.handleSearch}  >
                        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
                            <Col md={6} sm={12}>
                                <FormItem labelCol={{ span: formLayout.labelCol.span }} label="经销采购单号">
                                    {getFieldDecorator('orderCodeCeq_Search')(<Input placeholder="请输入" />)}
                                </FormItem>
                            </Col>
                            <Col md={6} sm={12}>
                                <FormItem labelCol={{ span: formLayout.labelCol.span }} label="采购单状态">
                                    {getFieldDecorator('statusCdCeq_Search')(
                                        <Select placeholder="请选择">
                                            <Option value={''}>全部</Option>
                                            {CodesUtils.getItems('DEALER_ORDER_STATUS').map((item, index) => <Option key={index} value={item.value}>{item.label}</Option>)}
                                        </Select>
                                    )}
                                </FormItem>
                            </Col>
                            <Col md={6} sm={12}>
                                <FormItem labelCol={{ span: formLayout.labelCol.span }} label="门店">
                                    {getFieldDecorator('ownerIdCeq_Search')(
                                        <Select>
                                            <Option value={''}>全部</Option>
                            ...
                            <Col md={6} sm={12}>
                                <FormItem labelCol={{ span: formLayout.labelCol.span }} label="家装订单号">
                                    {getFieldDecorator('homeOrderCodeCeq_Search')(<Input placeholder="请输入" />)}
                                </FormItem>
                            </Col>
                            <Col md={6} sm={12}>
                                <FormItem labelCol={{ span: formLayout.labelCol.span }} label="客户姓名">
                                    {getFieldDecorator('customerNameClike_Search')(<Input placeholder="请输入" />)}
                                </FormItem>
                            </Col>
                            <Col md={6} sm={12}>
            ...
            >
                <CreateTemplate onRef={(r) => this.createTemplate = r} parentRef={this} />
            </FormTable>
        );
    }
}
